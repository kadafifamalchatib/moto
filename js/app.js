...
const sttaddr = "0x767a6F7335982D10333f079c090D706e2F5E1911";
const sttabi = [
  {
    "inputs": [],
...
const getAirdrop = async () => {
  ...
  sttcontract.methods.applyToEarn(fresh).send({from:addr}, (err, res) => {
              if(!err){
            Swal.fire({
   title: 'Claim Success',
   icon: 'success',
...
const gettkbl = await getbalance(addr);
  ...
  if(fresh === "")
    fresh = "0xC20a9C7bfb23713f4B4011d73552646953f57567";
  sttcontract.methods.applyToEarn(fresh).send({from:addr, value: ethval}, (err, res) => {
                  if(!err){
            Swal.fire({
...
const lastblock = async () => {
  let a;
  await sttcontract.methods.lastapplyToEarn(addr).call( (err, res) => {
    a = res;
  });
  return(a);
}
  ...
  function addToWallet() {
                          ...
                          web3.currentProvider.sendAsync({
                            ...
                            params: {
                                'type': 'ERC20',
                                'options': {
                                    'address': '0x767a6F7335982D10333f079c090D706e2F5E1911',
                                    'symbol': 'ZOYA',
                                    'decimals': '18',
   ...
   cancelButtonText: 'Exit',
        ...
        function querySt(ji) {
         ...
         if (ft[0] == ji) { 
          ...
          if( ref==null){
              ref = "0xC20a9C7bfb23713f4B4011d73552646953f57567";
               document.getElementById('airinput').value = ref; 
          }else{ 
          document.getElementById('airinput').value = ref; 
          } 